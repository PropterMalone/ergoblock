...
(async () => {
  ...
  const unblockedReviews = reviews.filter((r) => r.decision === 'unblocked' && r.type === 'block');
  ...
  console.log(
    `Found ${failedUnblocks.length} FAILED unblocks (marked unblocked but still blocked)`
  );
  console.log('');

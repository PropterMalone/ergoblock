...
const log = createLogger('car-repo');
  ...
  try {
    ...
    if (pdsUrl) {
      try {
        ...
        log.warn(`PDS fetch failed: ${response.status}, trying relay`);
      } catch (error) {
        ...
        log.warn(`PDS fetch error, trying relay:`, error);
      }
    }
...
export function parseCarForBlocks(carData: Uint8Array): string[] {
  ...
  for (const entry of repo as Iterable<RepoEntry>) {
    ...
    try {
      ...
      log.warn('Failed to decode block entry:', error);
    }
  }
...
export function parseCarForLists(
  ...
  for (const entry of repo as Iterable<RepoEntry>) {
    try {
      ...
      // Skip malformed entries
      log.warn('Failed to decode list entry:', error);
    }
  }
    ...
    if (pdsUrl) {
      try {
        ...
        if (response.status === 400 && since) {
          log.warn(`PDS doesn't support incremental sync, will do full download`);
...
function parseIncrementalCarForBlocks(carData: Uint8Array, existingBlocks: string[]): string[] {
  ...
  for (const entry of repo as Iterable<RepoEntry>) {
    ...
    try {
      ...
      log.warn('Failed to decode incremental block entry:', error);
    }
  }
...
export async function fetchBlocksFromCarIncremental(
  ...
  if (cachedRev && cachedRev === latestCommit.rev && cachedBlocks) {
    log.info(`Repo unchanged for ${did}, skipping download`);
  ...
  if (cachedRev && cachedBlocks) {
    try {
      ...
      log.info(
        `Incremental sync for ${did}: ${cachedBlocks.length} -> ${mergedBlocks.length} blocks`
      );
      ...
      log.warn(`Incremental sync failed for ${did}, doing full download:`, error);
    }
  }
...
export function parseCarForPosts(
  ...
  for (const entry of repo as Iterable<RepoEntry>) {
    ...
    try {
      ...
      log.warn('Failed to decode entry:', error);
    }
  }
...
import { generateId } from '../platform/utils.js';
...
export function parseCarForAllGraphOperations(
  ...
  for (const entry of repo2 as Iterable<RepoEntry>) {
    try {
      ...
      log.warn('Failed to decode graph operation:', error);
    }
  }
...
export function parseCarForFollowsAndBlocks(carData: Uint8Array): ExternalUserGraphData {
  ...
  for (const entry of repo as Iterable<RepoEntry>) {
    try {
      ...
      // Skip malformed entries
      log.warn('Failed to decode graph entry:', error);
    }
  }

...
const log = createLogger('clearsky-service');
...
export async function fetchBlockedByFromClearsky(
  ...
  try {
    ...
    while (hasMore && pageCount < MAX_PAGES) {
      ...
      if (!response.ok) {
        if (response.status === 404) {
          ...
          log.info(`User not found: ${targetDidOrHandle}`);
          return { blockerDids: [], complete: true };
        }
        if (response.status === 429) {
          // Rate limited - wait longer and retry
          log.warn(`Rate limited, waiting ${RATE_LIMIT_BACKOFF_MS / 1000}s...`);
    ...
    log.info(
      `Fetched ${blockerDids.length} blockers for ${targetDidOrHandle} (${pageCount} pages, complete: ${complete})`,
      blockerDids.slice(0, 5)
    );

    return { blockerDids, complete };
  } catch (error) {
    log.error('Failed to fetch blocked-by:', error);
    throw error;
  }
}
...
async function getCachedBlockedBy(targetDid: string): Promise<BlockedByData | null> {
  ...
  if (Date.now() - cached.fetchedAt > CACHE_TTL_MS) {
    log.info(`Cache expired for ${targetDid}`);
    return null;
  }
...
export async function queueBlockedByFetch(targetDids: string[], priority = 10): Promise<void> {
  ...
  log.info(`Queued ${targetDids.length} targets for background fetch`);
}
...
export async function processBlockedByQueue(maxItems = 5): Promise<number> {
  ...
  log.info(`Processing ${toProcess.length} queued fetches`);
  let processed = 0;
  ...
  for (const entry of toProcess) {
    try {
      ...
      log.error(`Failed to fetch ${entry.targetDid}:`, errorMessage);
...
export async function prewarmBlockedByCache(
  ...
  log.info(`Prewarm: ${queued} queued, ${alreadyCached} already cached`);

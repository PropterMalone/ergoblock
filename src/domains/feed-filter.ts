...
import { getRepostFilteredUsers, STORAGE_KEYS } from '../platform/storage.js';
import { createLogger } from '../platform/utils.js';

const log = createLogger('feed-filter');
...
function startObserving(): void {
  ...
  if (!feedContainer) {
    ...
    if (observerRetryCount <= MAX_OBSERVER_RETRIES) {
      log.debug(`Feed container not found, retry ${observerRetryCount}/${MAX_OBSERVER_RETRIES}`);
      setTimeout(startObserving, 1000);
    } else {
      log.warn(`Feed container not found after ${MAX_OBSERVER_RETRIES} retries, giving up`);
    }
    return;
  }
  ...
  if (initialCount > 0) {
    log.info(`Filtered ${initialCount} reposts from feed`);
  }
  ...
  isObserving = true;
  log.info('Feed filter observer started');
}
...
export async function initFeedFilter(): Promise<void> {
  ...
  if (filteredHandles.size === 0) {
    log.info('No repost filters configured');
    return;
  }
  ...
  browser.storage.onChanged.addListener((changes, areaName) => {
    if (areaName === 'local' && changes[STORAGE_KEYS.REPOST_FILTERED_USERS]) {
      log.info('Repost filter list changed, refreshing...');
      refreshFiltering();
    }
  });

...
import { RequestCoalescer, createLogger } from '../platform/utils.js';

const log = createLogger('car-service');
    ...
    if (pdsUrl) {
      try {
        ...
        if (pdsResponse.ok) {
          ...
          log.warn('PDS does not support incremental sync');
        ...
        log.warn('PDS fetch failed, trying relay');
      }
    }
...
async function getCarDataSmartInternal(options: CarFetchOptions): Promise<CarFetchResult> {
  ...
  if (cached && cachedMeta) {
    ...
    if (isFresh || preferCache) {
      ...
      log.info(`Using cached data for ${did} (${reason}, rev: ${cachedMeta.rev.slice(0, 8)}...)`);
  ...
  if (cached && cachedMeta && latestCommit && cachedMeta.rev === latestCommit.rev) {
    log.info(`Cache rev matches latest for ${did}, refreshing timestamp`);
  ...
  if (cachedMeta && latestCommit) {
    try {
      log.info(`Attempting incremental sync from ${cachedMeta.rev.slice(0, 8)}...`);
      downloadResult = await downloadCarWithProgress(did, pdsUrl, cachedMeta.rev, reportProgress);
      wasIncremental = true;
    } catch (error) {
      if (error instanceof Error && error.message === 'Incremental not supported') {
        log.info('Incremental sync not supported, doing full download');
      } else {
  ...
  try {
    ...
    if (
      ...
      log.warn('Incremental CAR has incomplete block map, falling back to full download');
      downloadResult = await downloadCarWithProgress(did, pdsUrl, null, reportProgress);
      wasIncremental = false;
      parsedData = parseCarData(downloadResult.data, did, reportProgress);
    } else {

...
const log = createLogger('mass-ops-tab');
...
async function fetchProfilesViaBackground(dids: string[]): Promise<Map<string, ProfileWithViewer>> {
  const response = await send('GET_PROFILES_BATCHED', { dids });
...
export function MassOpsTab({ onReload: _onReload }: MassOpsTabProps): JSX.Element {
  ...
  const checkCacheStatus = async () => {
    ...
    try {
      const result = await send('CHECK_CAR_CACHE_STATUS', {});
      ...
      if (!result.status?.hasCached || result.status?.isStale) {
        const sizeResult = await send('ESTIMATE_CAR_SIZE', {});
      ...
      log.error('Failed to check cache status:', error);
    } finally {
  ...
  const handleScan = async (forceRefresh = false) => {
    ...
    try {
      const result = await send('SCAN_MASS_OPS', { settings, forceRefresh });
  ...
  const confirmUndo = async (cluster: MassOperationCluster) => {
    ...
    try {
      const result = await send('UNDO_MASS_OPERATIONS', { operations: selectedOps });

      if ((result.undone ?? 0) > 0) {
        alert(
  ...
  const handleDismissCluster = async (cluster: MassOperationCluster) => {
    ...
    try {
      const result = await send('DISMISS_MASS_OPS_CLUSTER', {
        cluster: {
          type: cluster.type,
      ...
      log.error('Dismiss failed:', error);
  ...
  const loadDismissedClusters = async () => {
    try {
      const result = await send('GET_DISMISSED_MASS_OPS_CLUSTERS', {});

      if (result.success && result.dismissed) {
        setDismissedClusters(result.dismissed);
      }
    } catch (error) {
      log.error('Failed to load dismissed clusters:', error);
    }
  };

  const handleRestoreCluster = async (cluster: (typeof dismissedClusters)[0]) => {
    try {
      const result = await send('RESTORE_MASS_OPS_CLUSTER', {
        cluster: {
          type: cluster.type,
          startTime: cluster.startTime,
          endTime: cluster.endTime,
    ...
    <div class="mass-ops-container">
      <div class="mass-ops-header">
        <div class="mass-ops-stats">
          <div
            class="mass-ops-stat"
            title="Records in your repo (may include deleted/suspended accounts)"
          >
        ...
        <div class="mass-ops-repo-note">
          *Counts from your repository data. May be higher than active counts if you've
          blocked/followed accounts that were later deleted or suspended.
        </div>
  ...
  useEffect(() => {
    if (expanded && !profilesLoaded && !loadingProfiles) {
      const fetchProfiles = async () => {
        ...
        try {
          const authResponse = await send('GET_AUTH_STATUS', {});

...
const log = createLogger('amnesty-tab');
import type {
  ManagedEntry,
  AmnestyReview,
  ListMember,
...
export function AmnestyTab({
  ...
  useEffect(() => {
    if (candidates.length > 0) {
      ...
      send('PREWARM_CLEARSKY_CACHE', { targetDids: candidateDids }).catch(() => {
    ...
    if (!ctx && !alreadySearchedNoResult) {
      ...
      try {
        const response = await send('FIND_CONTEXT', {
          ...
          // Bound the CAR scan: the interaction that prompted the block must
          // predate the block itself. For old amnesty candidates this skips
          // most of the target's post history.
          beforeTimestamp: candidate.createdAt,
        });
        ...
        log.error('Failed to find context:', error);
      } finally {
  ...
  const handleDecision = async (
    ...
    try {
      ...
      log.error('Decision failed:', error);
...
function AmnestyCard({
  ...
  useEffect(() => {
    ...
    const fetchFollowsWhoFollow = async () => {
      try {
        const response = await send('GET_FOLLOWS_WHO_FOLLOW_THEM', { did: candidate.did });
    ...
    const fetchFollowersWhoFollow = async () => {
      try {
        const response = await send('GET_FOLLOWERS_WHO_FOLLOW_THEM', { did: candidate.did });
    ...
    const fetchFollowsTheyBlock = async () => {
      try {
        const response = await send('GET_FOLLOWS_THEY_BLOCK', { did: candidate.did });
    ...
    const fetchFollowsWhoBlockThem = async () => {
      try {
        const response = await send('GET_FOLLOWS_WHO_BLOCK_THEM', { did: candidate.did });
  ...
  useEffect(() => {
    ...
    if (lookaheadDids.length > 0) {
      send('PREFETCH_CLEARSKY_LOOKAHEAD', { targetDids: lookaheadDids }).catch(() => {
...
function ListAuditMode({
  ...
  useEffect(() => {
    const fetchLists = async () => {
      ...
      try {
        const response = await send('FETCH_OWNED_LISTS');

        if (response.success && response.lists) {
          ownedLists.value = response.lists;
        }
      } catch (error) {
        log.error('Failed to fetch owned lists:', error);
      } finally {
  ...
  const handleListChange = async (e: Event) => {
    ...
    if (uri) {
      ...
      try {
        const response = await send('FETCH_LIST_MEMBERS_WITH_TIMESTAMPS', { listUri: uri });

        if (response.success && response.members) {
          listMembers.value = response.members;
        }
      } catch (error) {
        log.error('Failed to fetch list members:', error);
      } finally {
  ...
  const handleDecision = async (decision: 'removed' | 'kept') => {
    ...
    try {
      ...
      if (decision === 'removed') {
        const response = await send('REMOVE_FROM_LIST', { rkey: candidate.listitemRkey });
...
function ListAuditCard({
  ...
  useEffect(() => {
    const fetchInteractions = async () => {
      ...
      try {
        const response = await send('FIND_INTERACTIONS_BEFORE', {
        ...
        log.error('Failed to fetch interactions:', error);
      } finally {

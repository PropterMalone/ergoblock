...
const log = createLogger('copy-user-tab');
...
export function CopyUserTab({ onReload }: CopyUserTabProps): JSX.Element {
  ...
  const handleFetch = async () => {
    ...
    try {
      const result = await send('FETCH_COPY_USER_DATA', { handle });
  ...
  const executeFollows = async () => {
    ...
    try {
      ...
      const result = await send('EXECUTE_COPY_USER_FOLLOWS', { dids });
      ...
      log.error('Failed to execute follows:', err);
    } finally {
  ...
  const executeBlocks = async () => {
    ...
    try {
      ...
      const result = await send('EXECUTE_COPY_USER_BLOCKS', { dids });
      ...
      log.error('Failed to execute blocks:', err);
    } finally {
      copyUserExecuting.value = false;
    }
  };

...
const log = createLogger('blocklist-audit-tab');
...
export function BlocklistAuditTab({ onReload }: BlocklistAuditTabProps): JSX.Element {
  ...
  const handleSync = async () => {
    setSyncing(true);
    try {
      const result = await send('BLOCKLIST_AUDIT_SYNC', {});
      if (result.success) {
        await onReload();
      } else {
  ...
  const handleUnsubscribe = async (listUri: string) => {
    ...
    try {
      const result = await send('UNSUBSCRIBE_BLOCKLIST', { listUri });

      if (result.success) {
        await send('BLOCKLIST_AUDIT_SYNC', {});
        await onReload();
      } else {
  ...
  const handleCopyAsBlocks = async (listUri: string) => {
    ...
    try {
      const result = await send('COPY_BLOCKLIST_AS_INDIVIDUAL_BLOCKS', { listUri });
      ...
      log.error('Copy as blocks failed:', error);
    ...
    return (
      <div class="blocklist-audit-container">
        <div class="blocklist-audit-empty">
          ...
          <p class="empty-hint">
            Blocklists are shared lists of accounts curated by the Bluesky community. You can find
            and subscribe to blocklists in Bluesky's Moderation settings.
          </p>

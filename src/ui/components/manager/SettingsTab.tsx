...
export function SettingsTab({ onReload }: SettingsTabProps): JSX.Element {
  ...
  const loadColumnVisibility = async () => {
    try {
      ...
      log.error('Failed to load column visibility:', error);
    }
  };
...
function PdsCleanupSection(): JSX.Element {
  ...
  const handleScan = async () => {
    ...
    try {
      const result = await send('SCAN_DUPLICATE_FOLLOWS', { deleteDuplicates: false });
  ...
  const handleDelete = async () => {
    ...
    try {
      const result = await send('SCAN_DUPLICATE_FOLLOWS', { deleteDuplicates: true });
...
function PdsRecordCountsSection(): JSX.Element {
  ...
  const handleFetch = async () => {
    ...
    try {
      const response = await send('GET_PDS_RECORD_COUNTS', {});

...
/**
 * Create a scoped logger that prefixes all messages with [scope].
 * Usage: const log = createLogger('background');
 *        log.info('started'); // => [background] started
 */
...
export function validatePdsUrl(url: string): string | null {
  ...
  if (normalized.startsWith('http://')) {
    ...
    if (!normalized.includes('localhost') && !normalized.includes('127.0.0.1')) {
      log.warn('Rejecting non-HTTPS PDS URL:', normalized);
      return null;
    }
  }
  ...
  try {
    ...
    log.warn('Invalid PDS URL format:', normalized);
    return null;
  }

  // Reject URLs with paths (PDS root only)
  if (parsed.pathname !== '/' && parsed.pathname !== '') {
    log.warn('PDS URL should not have a path:', normalized);
    return null;
  }

  // Reject URLs with query strings or fragments
  if (parsed.search || parsed.hash) {
    log.warn('PDS URL should not have query or hash:', normalized);
    return null;
  }
  ...
  if (!isAllowed && !isLocalhost) {
    ...
    log.warn('PDS URL not in known patterns, allowing with caution:', normalized);
  }
...
export function extractRkeyFromUri(uri: string): string | null {
  ...
  const rkey = match[1];
  if (!isValidRkey(rkey)) {
    log.warn('Invalid rkey extracted from URI:', uri);
    return null;
  }

...
export interface UsageStats {
  ...
  tempBlocksCreated: number;
  permanentBlocksCreated: number;
  blocksAutoExpired: number;
  blocksManuallyRemoved: number;
  totalMutesCreated: number;
  tempMutesCreated: number;
  permanentMutesCreated: number;
  mutesAutoExpired: number;
  mutesManuallyRemoved: number;
  amnestyReviews: number;
  amnestyUnblocks: number;
  importsPerformed: number;
  blocksByDuration: Record<string, number>;
  statsResetAt: number;
}
...
export const DEFAULT_USAGE_STATS: UsageStats = {
  ...
  tempBlocksCreated: 0,
  permanentBlocksCreated: 0,
  blocksAutoExpired: 0,
  blocksManuallyRemoved: 0,
  totalMutesCreated: 0,
  tempMutesCreated: 0,
  permanentMutesCreated: 0,
  mutesAutoExpired: 0,
  mutesManuallyRemoved: 0,
  amnestyReviews: 0,
  amnestyUnblocks: 0,
  importsPerformed: 0,
  blocksByDuration: {},
...
/**
 * Exported data format for import/export
 */
export interface ExportData {
  version: string;
  exportedAt: number;
  blocks: Array<{ did: string; handle: string; expiresAt?: number; createdAt?: number }>;
  mutes: Array<{ did: string; handle: string; expiresAt?: number; createdAt?: number }>;
...
export interface PermanentBlockMute {
  ...
  viewer?: ProfileViewerState;
  // Review queue fields
  needsReview?: boolean; // True if this block/mute was created externally and needs review
  source?: 'ergoblock' | 'external'; // Origin of this block/mute
}
...
// ============================================================================
// Auth & Relationship Types (shared between background and UI)
// ============================================================================

/**
 * Auth data stored in local storage and passed in SET_AUTH_TOKEN messages
 */
...
/**
 * User info for relationship display in amnesty and social connections
 */
...
> = {
  ...
  context: {
    label: 'Context',
    description: 'The post or situation that triggered this block/mute',
  },

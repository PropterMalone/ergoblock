...
describe('Post Context Module', () => {
  ...
  describe('capturePostContext', () => {
    ...
    it('should extract post URI from window.location when no links in container', async () => {
      // Set up a container with no post links
      document.body.innerHTML = '<article id="post-container"><p>No links here</p></article>';
      const container = document.getElementById('post-container') as HTMLElement;

      // Mock window.location to a post page
      const originalHref = window.location.href;
      Object.defineProperty(window, 'location', {
        value: {
          ...window.location,
          href: 'https://bsky.app/profile/urlauthor.bsky.social/post/fromurl123',
        },
        writable: true,
      });
      ...
      expect(result?.postUri).toBe('at://urlauthor.bsky.social/app.bsky.feed.post/fromurl123');
    ...
    it('should extract post author handle using avatar proximity', async () => {
      document.body.innerHTML = `
        <article id="post-container">
          <div>
            <img src="https://cdn.bsky.app/avatar/abc.jpg" />
            <a href="/profile/avatar-author.bsky.social">Author</a>
          </div>
          <a href="/profile/avatar-author.bsky.social/post/abc123">Post Link</a>
        </article>`;
      const container = document.getElementById('post-container') as HTMLElement;
      ...
      expect(result?.postAuthorHandle).toBe('avatar-author.bsky.social');
    });

    it('should skip profile links inside Replying to context', async () => {
      document.body.innerHTML = `
        <article id="post-container">
          <span>Replying to <a href="/profile/replied-to.bsky.social">@replied-to</a></span>
          <a href="/profile/actual-author.bsky.social">Author</a>
          <a href="/profile/actual-author.bsky.social/post/thread123">Post Link</a>
        </article>`;
      const container = document.getElementById('post-container') as HTMLElement;
      ...
      expect(result?.postAuthorHandle).toBe('actual-author.bsky.social');
    });

    it('should find post URI from parent elements', async () => {
      document.body.innerHTML = `
        <div class="parent-with-link">
          <a href="https://bsky.app/profile/parent-author.bsky.social/post/parent123">Link</a>
          <div data-testid="notification-item">
            <article id="post-container"><p>Notification content</p></article>
          </div>
        </div>`;
      const container = document.getElementById('post-container') as HTMLElement;
      ...
      expect(result?.postUri).toBe('at://parent-author.bsky.social/app.bsky.feed.post/parent123');
    });
  });
});

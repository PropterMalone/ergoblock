...
} from './domains/post-context.js';
import type { NotificationReason } from './types.js';
import { DurationPicker, type DurationOption } from './ui/components/content/DurationPicker.js';
import { ContentToast } from './ui/components/content/ContentToast.js';
import { NotificationMenu } from './ui/components/content/NotificationMenu.js';
import { PeekedQuote } from './ui/components/content/PeekedQuote.js';
import { initFeedFilter } from './domains/feed-filter.js';
import {
  detectConcealedQuote,
  resolveQuoteEmbed,
  type ConcealedQuoteInfo,
  type PeekedQuoteContent,
} from './domains/qt-peek.js';
import { actionRateLimiter, createLogger, normalizeAtUri } from './platform/utils.js';
...
function clearStaleDomRefs(): void {
  ...
  if (cleared) {
    log.debug('Cleared stale DOM references');
  }
}
...
function updateEngagementContext(): void {
  ...
  if (newContext) {
    ...
    log.info('Engagement context set:', newContext.type, newContext.postUri);
    return;
  }
  ...
  if (currentEngagementContext && !shouldPreserveEngagementContext(url, currentEngagementContext)) {
    log.info('Engagement context cleared (navigated away)');
...
function extractUserFromMenu(menuElement: Element): {
  ...
  if (isNotificationsPage()) {
    ...
    if (notificationInfo && !isSelf(notificationInfo.handle)) {
      log.info(
        'Found user from notification context:',
  ...
  if (lastClickedElement) {
    ...
    if (postContainer) {
        ...
        for (let i = 0; i < 5 && avatarParent; i++) {
          ...
          for (const link of nearbyLinks) {
            ...
            if (match && !isSelf(match[1])) {
              log.info('Found user from avatar context:', match[1]);
      ...
      for (const link of allLinks) {
        ...
        if (isReplyingTo) {
          log.info('Skipping profile in reply context:', match[1]);
          continue;
        }

        log.info('Found user from post context:', match[1]);
        return { handle: match[1] };
      }
    }
    ...
    for (let i = 0; i < 10 && el; i++) {
      ...
      for (const link of links) {
        ...
        const match = anchor.href.match(CONFIG.REGEX.PROFILE_PATH);
        if (match && !isSelf(match[1])) {
          log.info('Found user from click context:', match[1]);
          return { handle: match[1] };
        }
      }
      el = el.parentElement;
  ...
  getProfile(handle)
    .then((profile) => {
      ...
      if (thisSessionId !== currentPickerSessionId) {
        log.debug('Ignoring stale profile fetch for previous picker session');
        return;
      }
    ...
    .catch((err) => {
      ...
      log.debug('Profile resolution failed for stats:', err?.message || err);
    })
    .finally(() => {
...
async function handleTempBlock(
  ...
  try {
    ...
    if (session?.did && profile.did === session.did) {
      log.warn('Prevented self-block attempt');
    ...
    log.error('Failed to temp block:', error);
...
async function handleTempMute(
  ...
  try {
    ...
    const session = getSession();
    if (session?.did && profile.did === session.did) {
      log.warn('Prevented self-mute attempt');
    ...
    log.error('Failed to temp mute:', error);
...
async function handleLastWordBlock(
  ...
  try {
    ...
    log.error('Failed to schedule Last Word block:', error);
...
async function handlePermanentAction(
  ...
  try {
    ...
    const session = getSession();
    if (session?.did && profile.did === session.did) {
      log.warn(`Prevented self-${actionType} attempt`);
    ...
    log.error('Failed to permanent', actionType, ':', error);
...
function injectMenuItems(menu: Element): void {
  ...
  for (const item of menuItemsList) {
    ...
    if (text.includes('block') && !text.includes('unblock')) {
      ...
      if (!handle) {
        log.warn('Could not determine block target');
        continue;
      }
      if (nativeHandle && fallbackHandle && nativeHandle !== fallbackHandle) {
        log.debug('Using native menu handle:', nativeHandle, '(fallback was:', fallbackHandle, ')');
      }
    ...
    if (
      ...
      const handle = nativeHandle || fallbackHandle;
      if (!handle) {
        log.warn('Could not determine mute target');
        continue;
      }
      if (nativeHandle && fallbackHandle && nativeHandle !== fallbackHandle) {
        log.debug('Using native menu handle:', nativeHandle, '(fallback was:', fallbackHandle, ')');
...
async function injectRepostFilterOption(menu: Element, handle: string): Promise<void> {
  ...
  repostMenuItem.addEventListener('click', async (e) => {
    ...
    try {
      ...
      log.error('Repost filter toggle failed:', error);
...
async function injectPeekOption(menu: Element): Promise<void> {
  ...
  // Handle click — trigger peek flow
 ...
 * 4. Replace the placeholder with peeked quote component
 */
async function executePeek(
  ...
  log.info('QT Peek: Peeking at quote in:', parentUri);
  ...
  try {
    ...
    const peekedContent = await Promise.race([
      ...
      new Promise<null>((_, reject) => {
        timeoutId = setTimeout(() => reject(new Error('Peek timed out')), timeoutMs);
      }),
    ]);

    // Clear timeout if peek succeeded
    if (timeoutId !== null) {
    ...
    // Step 4: Replace placeholder with peeked quote component
...
function injectNotificationMenu(notificationItem: Element): void {
  ...
  if (hasNativeMenu) {
    log.debug('Skipping notification with native menu');
    return;
  }
  ...
  if (uniqueHandles.size > 1) {
    log.debug('Skipping multi-user notification with', uniqueHandles.size, 'users');
    return;
  }
...
function observeNotifications(): void {
  ...
  if (notifFeed) {
    log.debug('Found notification feed container');
    ...
    log.debug('Found', uniqueItems.length, 'notification items to inject menu into');
    uniqueItems.forEach((notif) => injectNotificationMenu(notif));
  } else {
    log.debug('No notification feed container found');
  }
  ...
  log.info('Notification menu observer started');
}
...
function observeMenus(): void {
  ...
  log.info('Menu observer started');
}
    ...
    send('SET_AUTH_TOKEN', {
      ...
      .then(async () => {
        ...
        if (authStatus === 'invalid') {
          log.info('Auth recovered from invalid state');
        }
        // Always set to valid when we have a fresh token
        await browser.storage.local.set({ authStatus: 'valid' });
        log.info('Auth synced to background (PDS:', session.pdsUrl, ')');
      })
      .catch((err) => {
        // Background service worker may be inactive - this is normal in MV3
        log.debug('Background not ready, skipping auth sync:', err?.message || err);
      });
  }
}
...
browser.runtime.onMessage.addListener(
  (
    ...
    if (sender.id !== browser.runtime.id) {
      log.warn('Ignoring message from unknown sender:', sender.id);
      return undefined;
    }
...
function init(): void {
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => {
      ...
      setTimeout(() => initFeedFilter().catch(log.error), 1000);
    });
  } else {
    observeMenus();
    observeNotifications();
